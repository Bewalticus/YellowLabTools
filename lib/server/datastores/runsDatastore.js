...
function RunsDatastore() {
    ...
    this.markAsFailed = function(runId, err) {
        ...
        switch(err) {
            ...
            case '253':
                ...
                break;
            case '254':
                this.determinePageLoadingFailure(runId, run);
                return;
            case '255':
    ...
    this.determinePageLoadingFailure = function(runId, run) {
        
        try {
            var method = require('http');
            if (run.params.url.startsWith('https:')) {
                method = require('https');
            }
            var request = method.get(run.params.url, function(res) {
                var statusCode = res.statusCode;
                var errorMessage = "Error 254: page loading failed in PhantomJS (Status code: " + statusCode + ")";
                
                if (statusCode >= 301 || statusCode < 400) {
                    errorMessage = "Error 2001: Page was redirected (Code " + statusCode + "). Please try test with target URL " + res.headers.location;
                }
            ...
            request.on('error', function(err) {
                run.status = {
                    statusCode: STATUS_FAILED,
                    error: 'Error 254: page loading failed in PhantomJS (' + err + ')'
                };
                runs[runId] = run;
            });
        } catch (error) {
            run.status = {
                statusCode: STATUS_FAILED,
                error: 'Error 254: page loading failed in PhantomJS (' + error + ')'
            };
            runs[runId] = run;
        }
    };
